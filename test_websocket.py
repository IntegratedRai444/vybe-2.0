        ...
        while self.running and self.websocket:
            try:
                if time.time() - self.last_ping > self.ping_interval:
                    if await self.send_message({"type": "ping", "timestamp": int(time.time())}):
                        self.last_ping = time.time()
            except Exception as e:
        ...
        while self.running:
            try:
                ...
                if message.lower() == 'ping':
                    await self.send_message({"type": "ping", "timestamp": int(time.time())})
                elif message.lower() == 'subscribe':
                    await self.send_message({"type": "subscribe", "channels": ["updates"]})
...
async def test_http_connection():
    ...
    import aiohttp
    http_url = WS_URL.replace('ws://', 'http://').replace('wss://', 'https://').split('/ws')[0] or 'http://localhost:8000'
