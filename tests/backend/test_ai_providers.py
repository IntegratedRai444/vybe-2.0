...
async def mock_ai_orchestrator():
    ...
    with patch("ai_providers.NvidiaProvider.is_available", return_value=True), patch(
        "ai_providers.OpenAIProvider.is_available", return_value=True
    ), patch("ai_providers.AnthropicProvider.is_available", return_value=True), patch(
        "ai_providers.GroqProvider.is_available", return_value=True
    ), patch(
        "ai_providers.OllamaProvider.is_available", return_value=True
    ):
...
class TestNvidiaProvider:
    ...
    async def test_generate(self, nvidia_provider):
        ...
        with patch.object(
            nvidia_provider,
            "_generate_single",
            return_value=("Test response", {"total_tokens": 10}, 0.5, None),
        ):
...
class TestOllamaProvider:
    ...
    async def test_generate(self, ollama_provider):
        ...
        with patch.object(
            ollama_provider, "_make_request", return_value=(mock_response, 200)
        ):
...
class TestAIOrchestrator:
    ...
    async def test_get_available_providers(self, mock_ai_orchestrator):
        ...
        assert all(
            key in providers
            for key in ["nvidia", "openai", "anthropic", "groq", "ollama"]
        )
    ...
    async def test_generate_text(self, mock_ai_orchestrator):
        ...
        mock_response = AIResponse(
            content="Test response", model="test-model", provider="test-provider"
        )

        with patch.object(
            mock_ai_orchestrator.providers["nvidia"],
            "generate",
            return_value=mock_response,
        ):
            response = await mock_ai_orchestrator.generate_text(
                "nvidia", "Test prompt", "Test system prompt"
            )
    ...
    async def test_stream_text(self, mock_ai_orchestrator):
        ...
        with patch.object(
            ...
            chunks = []
            async for chunk in mock_ai_orchestrator.stream_text(
                "nvidia", "Test prompt"
            ):
                chunks.append(chunk)
...
class TestErrorHandling:
    ...
    async def test_provider_timeout(self, mock_ai_orchestrator):
        ...
        with patch.object(
            ...
            response = await mock_ai_orchestrator.generate_text("nvidia", "Test prompt")
    ...
    async def test_provider_error(self, mock_ai_orchestrator):
        ...
        with patch.object(
            mock_ai_orchestrator.providers["openai"],
            "generate",
            side_effect=Exception("API Error"),
        ) as mock_generate:
            response = await mock_ai_orchestrator.generate_text("openai", "Test prompt")
...
class TestConcurrency:
    ...
    async def test_concurrent_requests(self, mock_ai_orchestrator):
        ...
        with patch.object(
            mock_ai_orchestrator.providers["nvidia"], "generate", mock_generate
        ):
