...
class TestClass:
    pass
"""
    context = completion_service._get_code_context(code, 50, 'python')  # Position is after 'x = 1'
...
def example():
    ...
    # Mock AI completions
    with patch.object(completion_service, '_get_ai_completions', new_callable=AsyncMock) as mock_ai:
...
async def test_caching(completion_service):
    ...
    with patch.object(completion_service, '_generate_completions', new_callable=AsyncMock) as mock_gen:
