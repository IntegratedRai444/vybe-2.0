...
function updateExports(filePath) {
  ...
  if (
    isIndexFile ||
    !/export\s+(default\s+)?(function|const|class|interface|type)\s+\w+/i.test(
      content,
    )
  ) {
    return;
...
function updateIndexFiles() {
  ...
  function findDirs(dir) {
    ...
    const hasComponents = items.some(
      (item) =>
        !item.isDirectory() &&
  ...
  for (const dir of dirs) {
    ...
    const mainComponent = componentFiles.find(
      (f) => f.name === path.basename(dir),
    );
    if (mainComponent) {
