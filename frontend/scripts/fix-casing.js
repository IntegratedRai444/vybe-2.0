    ...
    if (item.isDirectory()) {
      ...
      if (base[0] === base[0].toLowerCase() && 
        ...
        if (!fs.existsSync(newPath)) {
          ...
          updateImports(path.relative(path.join(COMPONENTS_DIR, '..'), dir), base + ext, newName);
        }
      }
    }
  }
...
function updateImports(relativeDir, oldName, newName) {
  ...
  function processFile(filePath) {
    ...
    const oldImport = new RegExp(`from ['"](\.{1,2}\/)*${relativeDir.replace(/\//g, '\\/')}\/${oldName.replace(/\./g, '\\.')}['"]`, 'g');
  ...
  function processDirectory(dir) {
    ...
    for (const item of items) {
      ...
      } else if (item.name.endsWith('.ts') || item.name.endsWith('.tsx') || item.name.endsWith('.js') || item.name.endsWith('.jsx')) {
