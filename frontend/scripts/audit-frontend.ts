...
class FrontendAuditor {
  ...
  private async checkDependencies() {
    ...
    try {
      ...
      for (const [pkg, info] of Object.entries(outdated)) {
        this.issues.push({
          file: 'package.json',
          message: `Outdated package: ${pkg} (current: ${(info as any).current}, wanted: ${(info as any).wanted}, latest: ${(info as any).latest})`,
          severity: 'warning'
        });
      }
    }
  ...
  private async checkTypeScriptIssues() {
    ...
    try {
      ...
      const result = execSync('npx tsc --noEmit --pretty false', { stdio: 'pipe' });
  ...
  private async checkForDeprecatedImports() {
    ...
    for (const file of files) {
      ...
      for (const [deprecated, replacement] of Object.entries(deprecatedImports)) {
        if (content.includes(`from "${deprecated}"`) || content.includes(`from '${deprecated}'`)) {
          this.issues.push({
            file,
  ...
  private async checkForTypeErrors() {
    ...
    try {
      ...
      if (result.status !== 0) {
        ...
        const errorRegex = /([^\s].*?)\((\d+),(\d+)\): (error|warning) (TS\d+): (.*)/g;
        let match;
      ...
      for (const file of files) {
        try {
          const result = spawnSync('npx', [
            ...
            '--no-eslintrc',
            '--no-inline-config',
            '--rule', 'no-unused-vars: ["error", { "vars": "all", "args": "after-used", "ignoreRestSiblings": false }]',
            '--format', 'json',
            file
          ], {
            cwd: this.rootDir,
  ...
  private async checkForConsoleLogs() {
    ...
    try {
      ...
      for (const file of files) {
        try {
          const content = await readFile(path.join(this.rootDir, file), 'utf-8');
          ...
          lines.forEach((line, index) => {
            if (line.includes('console.log(') || 
              ...
              this.issues.push({
                file,
                message: 'Found console statement. Consider removing or using a proper logging library in production.',
                line: index + 1,
                severity: 'warning'
              });
            }
  ...
  private reportIssues() {
    ...
    const filesWithIssues = new Map<string, {errors: Issue[], warnings: Issue[], infos: Issue[]}>();
    
    this.issues.forEach(issue => {
      if (!filesWithIssues.has(issue.file)) {
        filesWithIssues.set(issue.file, { errors: [], warnings: [], infos: [] });
      }
    ...
    filesWithIssues.forEach((issues, file) => {
      ...
      const issueCounts = [
        ...
        warningCount > 0 ? `⚠️  ${warningCount} warning${warningCount > 1 ? 's' : ''}` : '',
      ...
      if (issues.errors.length > 0) {
        ...
        issues.errors.forEach(issue => {
          console.log(`    - ${issue.message}${issue.line ? ` (line ${issue.line}${issue.column ? `:${issue.column}` : ''})` : ''}`);
        });
      }
  ...
  private printIssues(issues: Issue[]) {
    ...
    for (const [file, fileIssues] of Object.entries(grouped)) {
      ...
      for (const issue of fileIssues) {
        ...
        console.log(`  ${issue.severity === 'error' ? '❌' : issue.severity === 'warning' ? '⚠️' : 'ℹ️'} ${issue.message}${location}`);
      }
    }
  }
}
