...
class IssueAnalyzer {
  ...
  private async runTypeScriptCheck() {
    ...
    try {
      ...
      if (error.status === 1 && error.stdout) {
        ...
        console.error(
          chalk.yellow("⚠️  TypeScript check failed:"),
          error.message,
        );
      }
    }
  ...
  private parseTypeScriptErrors(output: string) {
    ...
    for (const line of lines) {
      ...
      const fileMatch = line.match(
        /^(.*?):(\d+):(\d+)\s+-\s+(error|warning|suggestion)\s+(TS\d+):?\s*(.*)/,
      );
  ...
  private async checkForConsoleLogs() {
    ...
    for (const file of files) {
      ...
      const consoleRegex =
        /console\.(log|warn|error|info|debug|trace|time|timeEnd|timeLog|timeStamp|group|groupCollapsed|groupEnd|clear|count|countReset|dir|dirxml|table|assert|profile|profileEnd)(\s*\()/g;
  ...
  private analyzeResults() {
    ...
    for (const issue of this.issues) {
      // Count by category
      this.categories[issue.category] =
        (this.categories[issue.category] || 0) + 1;
