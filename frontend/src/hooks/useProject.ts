...
const useProject = ({
  ...
  // Save project
  const saveProject = useCallback(async (projectData?: Partial<ProjectState>) => {
  ...
  // Create new project
  const createProject = useCallback(async (projectName: string, projectPath: string) => {
    setState(prev => ({ ...prev, isLoading: true, error: null }));
  ...
  const addFile = useCallback((parentId: string | null, file: Omit<ProjectFile, 'id' | 'parentId' | 'isModified'>) => {
      ...
      setState(prev => {
        const removeFile = (files: ProjectFile[]): ProjectFile[] => {
          return files.reduce<ProjectFile[]>((acc, file) => {
            ...
            if (file.children) {
              return [...acc, { ...file, children: removeFile(file.children) }];
            }
    ...
    try {
      ...
      setState(prev => {
        const updateFileName = (files: ProjectFile[]): ProjectFile[] => {
          return files.map(file => {
            if (file.id === fileId) {
              return { ...file, name: newName, isModified: true, isRenamed: true };
            }
      ...
      const error = err instanceof Error ? err : new Error('Failed to rename file');
      setState(prev => ({ ...prev, error }));
      onError?.(error);
      throw error;
    }
  ...
  const updateSettings = useCallback((settings: Partial<ProjectState['settings']>) => {
    setState(prev => ({
      ...prev,
      settings: {
        ...prev.settings,
  ...
  const loadRecentProjects = useCallback(async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to load recent projects: ${response.statusText}`);
      }
      ...
      const error = err instanceof Error ? err : new Error('Failed to load recent projects');
      setState(prev => ({ ...prev, error }));
      onError?.(error);
      throw error;
    }
