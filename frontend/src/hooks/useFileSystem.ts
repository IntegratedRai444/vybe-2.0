...
const useFileSystem = ({
  ...
  const listDirectory = useCallback(
    async (path: string = currentPath) => {
      ...
      try {
        ...
        const error =
          err instanceof Error ? err : new Error("Failed to list directory");
        setError(error);
        throw error;
      } finally {
        setIsLoading(false);
  ...
  const writeFile = useCallback(
    async (filePath: string, content: string): Promise<void> => {
      try {
        ...
        const error =
          err instanceof Error ? err : new Error("Failed to write file");
        setError(error);
        throw error;
      }
    },
  ...
  const createDirectory = useCallback(
    async (dirPath: string): Promise<void> => {
      try {
        ...
        const error =
          err instanceof Error ? err : new Error("Failed to create directory");
        setError(error);
        throw error;
      }
    },
  ...
  const renameItem = useCallback(
    async (oldPath: string, newPath: string): Promise<void> => {
      try {
        ...
        const error =
          err instanceof Error ? err : new Error("Failed to rename item");
        setError(error);
        throw error;
      }
    },
  ...
  useEffect(() => {
    ...
    const watchForChanges = async () => {
      try {
        const response = await fetch(
          `/api/fs/watch?path=${encodeURIComponent(currentPath)}`,
          {
