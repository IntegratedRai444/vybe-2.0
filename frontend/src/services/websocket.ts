...
type WebSocketEventMap = {
  ...
  'reconnecting': { data: { attempt: number; maxAttempts: number; nextAttemptIn: number } };
...
type DeploymentEvent = BaseEvent<'deployment:started', {
...
type DeploymentProgressEvent = BaseEvent<'deployment:progress', {
  deployment_id: string;
  progress: number;
  message: string;
  timestamp: string;
...
type DeploymentLogsEvent = BaseEvent<'deployment:logs', {
  deployment_id: string;
  logs: string;
  is_error: boolean;
  timestamp: string;
...
type DeploymentCompletedEvent = BaseEvent<'deployment:completed', {
  deployment_id: string;
  status: 'completed';
  result: any;
  end_time: string;
...
type DeploymentFailedEvent = BaseEvent<'deployment:failed', {
  deployment_id: string;
  status: 'failed';
  error: string;
  end_time: string;
...
type GitStatusUpdatedEvent = BaseEvent<'git:status_updated', {
  repo_path: string;
  status: any; // Replace with your GitStatus type
  timestamp: string;
}>;

type GitBranchChangedEvent = BaseEvent<'git:branch_changed', {
  repo_path: string;
  branch: string;
  timestamp: string;
}>;

type GitCommitCreatedEvent = BaseEvent<'git:commit_created', {
  repo_path: string;
  commit: any; // Replace with your GitCommit type
  timestamp: string;
}>;

type PackageInstallStartedEvent = BaseEvent<'package:install_started', {
  package: string;
  version: string;
  status: 'installing';
  start_time: string;
...
type PackageInstallProgressEvent = BaseEvent<'package:install_progress', {
  package: string;
  progress: number;
  message: string;
  timestamp: string;
...
type PackageInstallCompletedEvent = BaseEvent<'package:install_completed', {
  package: string;
  status: 'installed';
  result: any;
  end_time: string;
...
type PackageInstallFailedEvent = BaseEvent<'package:install_failed', {
  package: string;
  status: 'failed';
  error: string;
  end_time: string;
...
type SubscriptionUpdatedEvent = BaseEvent<'subscription:updated', {
  channel: string;
...
class WebSocketClient {
    ...
    this.connectionPromise = new Promise<boolean>((resolve, reject) => {
      ...
      try {
        ...
        this.socket.onopen = () => {
          ...
          this.emit('connected', { data: { timestamp: new Date().toISOString() } });
        };
        ...
        this.socket.onclose = (event) => {
          ...
          if (this.isExplicitDisconnect) {
            this.emit('disconnected', { data: { code: event.code, reason: event.reason } });
  ...
  private emit<T extends EventType>(
    eventType: T,
    event: Omit<WebSocketEventMap[T], 'type' | 'timestamp'> & { [key: string]: unknown }
  ): void {
    const handlers = this.eventHandlers[eventType];
    if (handlers) {
