...
async function requestWithRetry(endpoint: string, options: RequestInit = {}, retries = 3, delay = 1000): Promise<Response> {
  try {
...
export const createFolder = (path: string) => request("/file/mkdir", { method: "POST", body: JSON.stringify({ path }) });

export function deleteFileOrFolder(path: string) {
  return request(`/fs/delete`, { method: 'POST', body: JSON.stringify({ path }) });
}
...
// ==================== AI & CHAT ====================
export const generateCode = (prompt: string, filePath: string, model?: string) => 
  request("/generate", { method: "POST", body: JSON.stringify({ prompt, file_path: filePath, model }) });
export const sendChatMessage = (message: string, root?: string, currentFile?: string, context?: string) => 
  request("/chat", { method: "POST", body: JSON.stringify({ message, root, current_file: currentFile, context }) });

// ==================== SYSTEM STATUS ====================
export const checkHealth = () => request("/health");
export const checkProviders = () => request("/providers/status");
export const checkBackendConnection = () => request("/health", { signal: AbortSignal.timeout(3000) });

// ==================== CODE OPERATIONS ====================
export const formatCode = (language: string, code: string, options?: any) => 
  request("/format", { method: "POST", body: JSON.stringify({ language, code, options }) });
export const lintFile = (filePath: string, projectRoot: string) => 
  request("/lint/file", { method: "POST", body: JSON.stringify({ file_path: filePath, project_root: projectRoot }) });
export const lintProject = (projectRoot: string) => 
  request("/lint/project", { method: "POST", body: JSON.stringify({ project_root: projectRoot }) });
export const getCodeCompletion = (code: string, cursorPos: number, filePath: string, language?: string) => 
  request("/complete", { method: "POST", body: JSON.stringify({ code, cursor_pos: cursorPos, file_path: filePath, language }) });
export const getHoverInfo = (code: string, cursorPos: number, filePath: string) => 
  request("/hover", { method: "POST", body: JSON.stringify({ code, cursor_pos: cursorPos, file_path: filePath }) });
export const getSignatureHelp = (code: string, cursorPos: number, filePath: string) => 
  request("/signature", { method: "POST", body: JSON.stringify({ code, cursor_pos: cursorPos, file_path: filePath }) });
export const findDefinition = (code: string, cursorPos: number, filePath: string) => 
  request("/definition", { method: "POST", body: JSON.stringify({ code, cursor_pos: cursorPos, file_path: filePath }) });
export const findReferences = (code: string, cursorPos: number, filePath: string) => 
  request("/references", { method: "POST", body: JSON.stringify({ code, cursor_pos: cursorPos, file_path: filePath }) });
...
// ==================== FILE WATCHING ====================
export const startWatching = (path: string) => request("/watch/start", { method: "POST", body: JSON.stringify({ path }) });
export const stopWatching = (path: string) => request("/watch/stop", { method: "POST", body: JSON.stringify({ path }) });
export const getWatchStatus = () => request("/watch/status");

// ==================== ENHANCED GIT OPERATIONS ====================
export const getGitRepoInfo = (root: string) => request(`/git/info?root=${encodeURIComponent(root)}`);
export const getCommitHistory = (root: string, limit: number = 50, branch?: string) => {
  const params = new URLSearchParams({ root, limit: limit.toString() });
  if (branch) params.append("branch", branch);
  return request(`/git/history?${params.toString()}`);
};
export const getDetailedBranches = (root: string, includeRemote: boolean = true) => 
  request(`/git/branches/detailed?root=${encodeURIComponent(root)}&include_remote=${includeRemote}`);
export const getFileBlame = (root: string, filePath: string) => 
  request(`/git/blame?root=${encodeURIComponent(root)}&file_path=${encodeURIComponent(filePath)}`);
export const getStashList = (root: string) => request(`/git/stash?root=${encodeURIComponent(root)}`);
export const createStash = (root: string, message?: string, includeUntracked: boolean = false) => 
  request("/git/stash/create", { method: "POST", body: JSON.stringify({ root, message, include_untracked: includeUntracked }) });
export const applyStash = (root: string, stashName: string = "stash@{0}") => 
  request("/git/stash/apply", { method: "POST", body: JSON.stringify({ root, stash_name: stashName }) });
export const getMergeConflicts = (root: string) => request(`/git/conflicts?root=${encodeURIComponent(root)}`);
export const resolveConflict = (root: string, filePath: string, resolution: "ours" | "theirs") => 
  request("/git/conflicts/resolve", { method: "POST", body: JSON.stringify({ root, file_path: filePath, resolution }) });

// ==================== DAP DEBUGGING ====================
export const createDAPSession = (language: string, program: string, args?: string[]) => 
  request("/dap/session/create", { method: "POST", body: JSON.stringify({ language, program, args }) });
...
export const attachDAPSession = (sessionId: string, port: number) => 
  request(`/dap/session/${sessionId}/attach`, { method: "POST", body: JSON.stringify({ port }) });
...
export const setDAPBreakpoints = (sessionId: string, filePath: string, breakpoints: Array<{line: number, condition?: string, enabled?: boolean}>) => 
  request("/dap/breakpoints", { method: "POST", body: JSON.stringify({ session_id: sessionId, file_path: filePath, breakpoints }) });
export const continueDAPExecution = (sessionId: string, threadId?: number) => 
  request(`/dap/session/${sessionId}/continue`, { method: "POST", body: JSON.stringify({ thread_id: threadId }) });
export const stepOverDAP = (sessionId: string, threadId?: number) => 
  request(`/dap/session/${sessionId}/step-over`, { method: "POST", body: JSON.stringify({ session_id: sessionId, thread_id: threadId }) });
export const stepIntoDAP = (sessionId: string, threadId?: number) => 
  request(`/dap/session/${sessionId}/step-into`, { method: "POST", body: JSON.stringify({ session_id: sessionId, thread_id: threadId }) });
export const stepOutDAP = (sessionId: string, threadId?: number) => 
  request(`/dap/session/${sessionId}/step-out`, { method: "POST", body: JSON.stringify({ session_id: sessionId, thread_id: threadId }) });
export const pauseDAPExecution = (sessionId: string, threadId?: number) => 
  request(`/dap/session/${sessionId}/pause`, { method: "POST", body: JSON.stringify({ session_id: sessionId, thread_id: threadId }) });
...
export const getDAPVariables = (sessionId: string, variablesReference: number) => 
  request(`/dap/session/${sessionId}/variables/${variablesReference}`);
export const evaluateDAPExpression = (sessionId: string, expression: string, frameId?: number) => 
  request(`/dap/session/${sessionId}/evaluate`, { method: "POST", body: JSON.stringify({ session_id: sessionId, expression, frame_id: frameId }) });

// ==================== LEGACY DEBUGGING (Keep for compatibility) ====================
export const createDebugSession = (filePath: string, language?: string) => 
  request("/debug/session/create", { method: "POST", body: JSON.stringify({ file_path: filePath, language }) });
export const listDebugSessions = () => request("/debug/sessions");
export const getDebugSession = (sessionId: string) => request(`/debug/session/${sessionId}`);
export const terminateDebugSession = (sessionId: string) => request(`/debug/session/${sessionId}`, { method: "DELETE" });
export const setBreakpoint = (sessionId: string, line: number, condition?: string) => 
  request("/debug/breakpoint/set", { method: "POST", body: JSON.stringify({ session_id: sessionId, line, condition }) });
export const removeBreakpoint = (sessionId: string, line: number) => 
  request("/debug/breakpoint/remove", { method: "POST", body: JSON.stringify({ session_id: sessionId, line }) });
export const startDebugging = (sessionId: string) => 
  request("/debug/start", { method: "POST", body: JSON.stringify({ session_id: sessionId }) });
export const pauseDebugging = (sessionId: string) => 
  request("/debug/pause", { method: "POST", body: JSON.stringify({ session_id: sessionId }) });
export const resumeDebugging = (sessionId: string) => 
  request("/debug/resume", { method: "POST", body: JSON.stringify({ session_id: sessionId }) });
export const stepOver = (sessionId: string) => 
  request("/debug/step/over", { method: "POST", body: JSON.stringify({ session_id: sessionId }) });
export const stepInto = (sessionId: string) => 
  request("/debug/step/into", { method: "POST", body: JSON.stringify({ session_id: sessionId }) });
export const stepOut = (sessionId: string) => 
  request("/debug/step/out", { method: "POST", body: JSON.stringify({ session_id: sessionId }) });
...
export const evaluateExpression = (sessionId: string, expression: string) => 
  request("/debug/evaluate", { method: "POST", body: JSON.stringify({ session_id: sessionId, expression }) });
...
export const updateSetting = (keyPath: string, value: any, scope: "user" | "workspace" = "user", workspacePath?: string) => 
  request("/settings/update", { method: "POST", body: JSON.stringify({ key_path: keyPath, value, scope, workspace_path: workspacePath }) });
export const resetSettings = (scope: "user" | "default" = "user") => 
  request("/settings/reset", { method: "POST", body: JSON.stringify({ scope }) });
...
export const importSettings = (settings: any, scope: "user" = "user") => 
  request("/settings/import", { method: "POST", body: JSON.stringify({ settings, scope }) });
export const getKeybindings = () => request("/keybindings");
export const updateKeybinding = (command: string, keybinding: string) => 
  request("/keybindings/update", { method: "POST", body: JSON.stringify({ command, keybinding }) });

// ==================== PROJECT SEARCH ====================
export const searchInFiles = (root: string, query: string, options?: {
...
export const replaceInFiles = (root: string, query: string, replacement: string, options?: {
  caseSensitive?: boolean;
  wholeWord?: boolean;
  regex?: boolean;
  includePattern?: string;
...
export const startLSPServer = (language: string, rootPath: string) => 
  request("/lsp/start", { method: "POST", body: JSON.stringify({ language, root_path: rootPath }) });

export const getLSPCompletions = (filePath: string, line: number, character: number, language: string) => 
  request("/lsp/completions", { method: "POST", body: JSON.stringify({ file_path: filePath, line, character, language }) });

export const getLSPHover = (filePath: string, line: number, character: number, language: string) => 
  request("/lsp/hover", { method: "POST", body: JSON.stringify({ file_path: filePath, line, character, language }) });

export const getLSPDefinition = (filePath: string, line: number, character: number, language: string) => 
  request("/lsp/definition", { method: "POST", body: JSON.stringify({ file_path: filePath, line, character, language }) });

export const getLSPReferences = (filePath: string, line: number, character: number, language: string) => 
  request("/lsp/references", { method: "POST", body: JSON.stringify({ file_path: filePath, line, character, language }) });

export const notifyLSPDocumentOpen = (filePath: string, content: string, language: string) => 
  request("/lsp/document/open", { method: "POST", body: JSON.stringify({ file_path: filePath, content, language }) });

export const notifyLSPDocumentChange = (filePath: string, content: string, language: string, version: number = 1) => 
  request("/lsp/document/change", { method: "POST", body: JSON.stringify({ file_path: filePath, content, language, version }) });

export const notifyLSPDocumentClose = (filePath: string, language: string) => 
  request("/lsp/document/close", { method: "POST", body: JSON.stringify({ file_path: filePath, language }) });
