...
interface AISettingsState {
  ...
  toggleSettings: () => void;
  setProviderAvailability: (provider: ProviderType, isAvailable: boolean) => void;
    ...
    persist(
      (set, get) => ({
        ...
        setProvider: (provider) => {
          const { models } = get().providers.find(p => p.id === provider) || { models: [] };
          set({
            provider,
            model: models[0]?.id || '',
          });
        ...
        setTemperature: (temperature) => set({ temperature: Math.max(0, Math.min(1, temperature)) }),
...
export const useAISettings = () => useAISettingsStore((state) => ({
  provider: state.provider,
  model: state.model,
  temperature: state.temperature,
...
export const useAISettingsActions = () => useAISettingsStore((state) => ({
