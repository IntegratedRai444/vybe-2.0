...
type FileInput = Omit<FileTab, 'id' | 'isDirty' | 'content' | 'language' | 'type' | 'lastModified' | 'lastOpened'> & {
  language?: string;
};
...
export const useStore = create<StoreState>((set, get) => ({
  ...
  setCursorPosition: (cursorPosition: CursorPosition) => set({ cursorPosition }),
  setCommandPaletteOpen: (isCommandPaletteOpen: boolean) => set({ isCommandPaletteOpen }),
  setConnectionStatus: (connectionStatus: 'connected' | 'disconnected' | 'connecting' | 'checking') => 
    ...
    set((state) => {
      ...
      if (!panelState.visible) {
        Object.keys(updatedPanels).forEach((key) => {
          if (updatedPanels[key].position === panelState.position && key !== panel) {
...
export const useActiveFile = () => useStore((state) => 
...
export const useIsCommandPaletteOpen = () => useStore((state) => state.isCommandPaletteOpen);
