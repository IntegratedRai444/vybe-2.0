      ...
      (set, get) => ({
        ...
        togglePanel: (panel) =>
          set((state) => ({
            layout: {
              ...
              panels: {
                ...Object.fromEntries(
                  Object.entries(state.layout.panels).map(([key, panelConfig]) => [
                    key,
                    { ...panelConfig, isActive: key === panel ? !panelConfig.isActive : false },
                  ])
                ),
                [panel]: {
        ...
        removeFile: (fileId) =>
          set((state) => {
            ...
            if (file?.isDirty && !confirm('You have unsaved changes. Are you sure you want to close this file?')) {
              return state;
            }
...
export const useActiveFile = () => 
  useStore((state) => state.files.find((file) => file.id === state.activeFile) || null);
