...
const showToast = (message: string, type: 'log' | 'error' | 'info' | 'success' = 'info') => {
...
export const GitProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchStatus = useCallback(async (showNotification = false) => {
    try {
      ...
      if (showNotification) {
        showToast(`Repository status updated. Branch: ${result.branch || 'main'}`, 'info');
      }
  ...
  const unstageFiles = useCallback(async (filePaths: string[]) => {
    try {
      setIsLoading(true);
      // Replace with actual Tauri API call
