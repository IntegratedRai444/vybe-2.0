...
import { debuggerApi, type DebugSessionConfig, type Breakpoint, type StackFrame, type Variable } from '../services/api/debuggerApi';
...
interface DebuggerContextType {
  // Session management
  startDebugging: (config: Omit<DebugSessionConfig, 'stopOnEntry' | 'env' | 'args' | 'cwd' | 'type'> & {
    stopOnEntry?: boolean;
  ...
  // Breakpoints
  addBreakpoint: (filePath: string, line: number, condition?: string) => Promise<Breakpoint>;
  updateBreakpoint: (filePath: string, breakpointId: string, updates: Partial<Omit<Breakpoint, 'id' | 'verified'>>) => Promise<Breakpoint>;
  removeBreakpoint: (filePath: string, breakpointId: string) => Promise<void>;
  getBreakpoints: (filePath?: string) => Promise<Breakpoint[]>;
  toggleBreakpoint: (filePath: string, breakpointId: string, enabled: boolean) => Promise<void>;
  ...
  evaluateExpression: (expression: string, frameId?: number, context?: 'watch' | 'repl' | 'hover') => Promise<any>;
  getVariables: (reference: number, frameId?: number, scope?: 'local' | 'global') => Promise<Variable[]>;
...
export const DebuggerProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const { currentSessionId, isActive, isPaused, breakpoints = {}, variables = {}, callStack = [], output = [], error, loading } = debuggerState;
  const eventHandlers = useRef<Record<string, ((data: any) => void)[]>>({});

  const startDebugging = useCallback(async (config: Omit<DebugSessionConfig, 'stopOnEntry' | 'env' | 'args' | 'cwd' | 'type'> & {
    stopOnEntry?: boolean;
    env?: Record<string, string>;
    args?: string[];
    cwd?: string;
    ...
    try {
      ...
      dispatch(setError(error instanceof Error ? error.message : 'Failed to start debug session'));
      throw error;
    } finally {
  ...
  const stopDebugging = useCallback(async () => {
    ...
    try {
      ...
      dispatch(setError(error instanceof Error ? error.message : 'Failed to stop debug session'));
      throw error;
    } finally {
      dispatch(setLoading(false));
    }
  ...
  const listSessions = useCallback(async () => {
    try {
      ...
      dispatch(setError(error instanceof Error ? error.message : 'Failed to list debug sessions'));
      throw error;
    }
  }, [dispatch]);

  // ... rest of the code remains the same ...
  const addBreakpoint = useCallback(async (filePath: string, line: number, condition?: string) => {
    if (!currentSessionId) throw new Error('No active debug session');
    
    try {
      const breakpoint = await debuggerApi.setBreakpoint(currentSessionId, filePath, {
        line,
        column: 0,
        condition,
        enabled: true,
  ...
  const updateBreakpoint = useCallback(async (filePath: string, breakpointId: string, updates: Partial<Omit<Breakpoint, 'id' | 'verified'>>) => {
    if (!currentSessionId) throw new Error('No active debug session');
    
    try {
      const updatedBreakpoint = await debuggerApi.updateBreakpoint(currentSessionId, breakpointId, updates);
  ...
  // Remove a breakpoint
  const removeBreakpoint = useCallback(async (filePath: string, breakpointId: string) => {
    if (!currentSessionId) throw new Error('No active debug session');
  ...
  const toggleBreakpoint = useCallback(async (filePath: string, breakpointId: string, enabled: boolean): Promise<void> => {
    if (!currentSessionId) {
    ...
    try {
      await debuggerApi.updateBreakpoint(currentSessionId, breakpointId, { enabled });
  ...
  // Get breakpoints
  const getBreakpoints = useCallback(async (filePath?: string) => {
    if (!currentSessionId) throw new Error('No active debug session');
  ...
  const continueExecution = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to continue execution';
      dispatch(setError(errorMessage));
      throw error;
    }
  }, [dispatch, currentSessionId]);
  ...
  const evaluateExpression = useCallback(async (expression: string, frameId?: number, context: 'watch' | 'repl' | 'hover' = 'repl') => {
    if (!currentSessionId) throw new Error('No active debug session');
    try {
      const result = await debuggerApi.evaluateExpression(currentSessionId, expression, frameId, context);
      return result;
    } catch (error) {
  ...
  const getVariables = useCallback(async (reference: number, frameId?: number, scope: 'local' | 'global' = 'local') => {
    if (!currentSessionId) {
      const error = new Error('No active debug session');
      dispatch(setError(error.message));
      throw error;
    ...
    try {
      dispatch(setLoading(true));
      const { variables } = await debuggerApi.getVariables(currentSessionId, frameId, scope, reference);
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to get variables';
  ...
  const getSource = useCallback(async (sourceReference: number) => {
    if (!currentSessionId) {
      const error = new Error('No active debug session');
      dispatch(setError(error.message));
      throw error;
    ...
    try {
      dispatch(setLoading(true));
      const { content } = await debuggerApi.getSource(currentSessionId, sourceReference);
      return content;
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Failed to get source';
...
export const useDebuggerEvent = (event: string, handler: (data: any) => void) => {
