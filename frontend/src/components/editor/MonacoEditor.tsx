...
export const MonacoEditor: React.FC<Props> = ({
  ...
  const notifyDocumentChange = useCallback(async (content: string) => {
    if (!isLSPReady) return;

    try {
      documentVersion.current += 1;
      await api.notifyLSPDocumentChange(filePath, content, getLanguageId(language), documentVersion.current);
    } catch (error) {
  ...
  const createCompletionProvider = useCallback((): monaco.languages.CompletionItemProvider => ({
    provideCompletionItems: async (_model, position) => {
      ...
      try {
        ...
        const suggestions = result.completions?.map((item: LSPCompletionItem) => ({
          label: item.label,
  ...
  // LSP Hover Provider
  const createHoverProvider = useCallback((): monaco.languages.HoverProvider => ({
  ...
  const createDefinitionProvider = useCallback((): monaco.languages.DefinitionProvider => ({
  ...
  const createReferencesProvider = useCallback((): monaco.languages.ReferenceProvider => ({
  ...
  useEffect(() => {
    if (isLSPReady && editorRef.current) {
      ...
      monaco.languages.registerDefinitionProvider(languageId, createDefinitionProvider());
      monaco.languages.registerReferenceProvider(languageId, createReferencesProvider());

      notifyDocumentOpen();
    }
  }, [isLSPReady, language, createCompletionProvider, createHoverProvider,
    createDefinitionProvider, createReferencesProvider, notifyDocumentOpen]);

  return (
    <div className="relative h-full">
      {/* LSP Status Indicator */}
      <div className="absolute top-2 right-2 z-10">
        <div className={`px-2 py-1 rounded text-xs font-medium ${isLSPReady
