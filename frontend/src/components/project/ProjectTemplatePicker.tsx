...
const ProjectTemplatePicker: React.FC<ProjectTemplatePickerProps> = ({ onSelectTemplate, onCancel }) => {
  ...
  const filteredTemplates = defaultTemplates.filter((template: ProjectTemplate) => {
    const matchesSearch = 
  ...
  const getTemplateIcon = (template: ProjectTemplate): JSX.Element => {
    ...
    } else if (template.tags.includes('node') || template.tags.includes('express')) {
...
{template.tags.slice(0, 3).map((tag: string) => (
                    ...
                    <div className="text-xs text-gray-500 mb-4">
                      ...
                      <ul className="space-y-1">
                        {template.files.slice(0, 3).map((file: { path: string }, idx: number) => (
                        ...
                        {template.files.length > 3 && (
                          <li className="text-gray-400">+{template.files.length - 3} more files</li>
                        )}
                      </ul>
                    </div>
                  </div>
