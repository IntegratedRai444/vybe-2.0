...
import { FiFolder, FiFile, FiChevronRight, FiChevronDown, FiPlus, FiTrash2 } from 'react-icons/fi';
...
const ProjectExplorer: React.FC = () => {
  const { currentProject, createFile, deleteFile, getFile, saveFile } = useProject();
  const [expandedFolders, setExpandedFolders] = useState<Set<string>>(new Set());
  const [newItem, setNewItem] = useState<{ type: 'file' | 'folder' | null; parentPath: string }>({ 
    type: null, 
    parentPath: '' 
  });
  ...
  const handleAddItem = useCallback((type: 'file' | 'folder', parentPath: string) => {
    ...
    return items.map(item => {
      ...
      return (
        <div key={item.path || 'root'} className="pl-4">
          <div 
            className={`flex items-center py-1 px-2 hover:bg-gray-100 rounded cursor-pointer ${!isFolder ? 'ml-4' : ''}`}
          ...
          >
            ...
            <span className="mr-2">
              {isFolder ? <FiFolder className="text-blue-500" /> : <FiFile className="text-gray-600" />}
            </span>
          ...
          {isFolder && isExpanded && item.children && (
            <div>
              {item.children.length > 0 ? (
                ...
                <div className="pl-8 text-gray-400 text-xs py-1">Empty folder</div>
              )}
            </div>
          )}
        </div>
