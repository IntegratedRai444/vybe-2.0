...
export const ChatPanel: React.FC<ChatPanelProps> = ({ onClose, className = '' }) => {
  ...
  const isConfigured = Boolean(provider && model && (provider === 'ollama' || apiKey));
  ...
  if (!isConfigured) {
    return (
      <div className={`h-full flex flex-col bg-[#1e1e1e] border-l border-gray-700 ${className}`}>
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
          <div className="bg-yellow-900/50 border border-yellow-800 text-yellow-200 p-4 rounded-lg max-w-md">
            ...
            <p className="text-sm text-yellow-300 mb-4">
              Please configure your AI settings to start using the chat assistant.
            </p>
            <Button
              variant="secondary"
              size="sm"
