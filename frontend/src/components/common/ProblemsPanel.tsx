...
export const ProblemsPanel: React.FC<Props> = ({ problems, onProblemClick }) => {
      ...
      <div className="flex-1 overflow-y-auto">
        {filteredProblems.length === 0 ? (
          <div className="p-4 text-center text-gray-500 text-sm">
            {problems.length === 0 ? "No problems detected ✨" : `No ${filter} found`}
          </div>
        ) : (
          filteredProblems.map((problem, index) => (
            <div
              ...
              onClick={() => onProblemClick(problem.file, problem.line, problem.column)}
            >
              <div className="flex items-start space-x-2">
                <div className="mt-0.5">{getSeverityIcon(problem.severity)}</div>
                <div className="flex-1 min-w-0">
                  <div className="text-sm text-white mb-1">{problem.message}</div>
                  <div className="text-xs text-gray-400 flex items-center space-x-2">
                    <span className="truncate">{problem.file.split(/[/\\]/).pop()}</span>
                    <span>Line {problem.line}, Column {problem.column}</span>
                    {problem.source && (
                      <span className="bg-gray-700 px-1 rounded">{problem.source}</span>
                    )}
                  </div>
                </div>
              </div>
