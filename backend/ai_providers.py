...
class OllamaProvider(AIProvider):
    ...
    async def is_available(self) -> bool:
        try:
            async with self.session.get(f"{self.host}/api/tags", timeout=3) as response:
    ...
    async def generate(
        ...
        **kwargs,
        ...
        try:
            ...
            return AIResponse(
                content="", model=model, provider=self.provider_name, error=str(e)
            )
    ...
    async def is_available(self) -> bool:
        ...
        try:
            ...
            async with self.session.get(
                f"{self.base_url}/models", headers=self._get_headers(), timeout=5
            ) as response:
                if response.status == 200:
                    return True
                ...
                # Handle rate limiting
                    ...
                    reset_time = int(
                        response.headers.get("x-ratelimit-reset-requests", 60)
                    )
...
class NvidiaProvider(AIProvider):
    ...
    def __init__(self, api_keys: Optional[Dict[str, str]] = None):
        ...
        self.model_stats = {
            model_id: {"requests": 0, "errors": 0, "avg_latency": 0.0, "last_used": 0.0}
    ...
    def _select_best_model(self) -> Optional[str]:
        ...
        for model_id in self.available_models:
            ...
            if total_requests > 0:
                ...
                score *= (
                    1.0 - error_rate * 0.5
                )  # Reduce score by up to 50% for error rate
    ...
    async def generate(
        ...
        **kwargs,
        ...
        default_params = self.model_configs[model or self.default_model][
            "default_params"
        ]
        ...
        tasks = [
            self._generate_single(
                model=model_id,
                messages=messages,
                params={**params, **self.model_configs[model_id]["default_params"]},
            )
    ...
    async def stream_generate(
        ...
        **kwargs,
        ...
        for word in response.split():
            yield AIResponse(
                content=word + " ", model=model, provider=self.provider_name
            )
...
class AIOrchestrator:
    ...
    def _select_providers(self, count: int = 2) -> List[str]:
        ...
        for name in self.available_providers:
            ...
            if stats["max_concurrent"] > 0:
                load_factor = 1.0 / (
                    1.0 + stats["concurrent_requests"] / max(1, stats["max_concurrent"])
                )
    ...
    def get_best_provider(self) -> str:
        ...
        return (
            available_providers[0][0] if available_providers else self.fallback_order[0]
        )
    ...
    async def _execute_with_provider(
        self, provider_name: str, prompt: str, system_prompt: str = "", **kwargs
    ) -> AIResponse:
        ...
        stats["max_concurrent"] = max(
            stats["max_concurrent"], stats["concurrent_requests"]
        )
