...
class DebugSession:
    ...
    def __init__(self, session_id: str, language: str, program: str, args: List[str] = None):
    ...
    async def send_request(self, command: str, arguments: Dict = None) -> Optional[Dict]:
...
class PythonDAPAdapter:
        ...
        try:
            ...
            session.process = await asyncio.create_subprocess_exec(
                *debug_cmd,
                ...
                cwd=os.path.dirname(session.program) if os.path.dirname(session.program) else "."
            )
    ...
    async def _handle_stopped_event(self, session: DebugSession, body: Dict):
        ...
        if thread_id:
            # Get stack trace
            stack_response = await session.send_request("stackTrace", {"threadId": thread_id})
            if stack_response:
                frames = []
...
class DAPManager:
    ...
    async def create_session(self, language: str, program: str, args: List[str] = None) -> Optional[str]:
    ...
    async def set_breakpoints(self, session_id: str, file_path: str, breakpoints: List[Dict]) -> List[Breakpoint]:
    ...
    async def get_stack_trace(self, session_id: str, thread_id: int) -> List[StackFrame]:
    ...
    async def get_variables(self, session_id: str, variables_reference: int) -> List[Variable]:
        """Get variables for scope"""
        session = self.sessions.get(session_id)
        if not session:
            return []
        
        response = await session.send_request("variables", {"variablesReference": variables_reference})
        if response:
            variables = []
    ...
    async def evaluate_expression(self, session_id: str, expression: str, frame_id: int = None) -> Optional[Dict]:
