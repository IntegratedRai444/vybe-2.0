...
export function setupAIRoutes(app) {
  ...
  app.post('/generate', async (req, res) => {
    try {
      ...
      const contextualPrompt = buildContextualPrompt(prompt, file_path, context);
      ...
      switch (provider) {
        case 'openai':
          ...
          response = await generateOpenAIResponse(contextualPrompt, model, temperature, max_tokens);
          break;
        ...
        case 'anthropic':
          ...
          response = await generateAnthropicResponse(contextualPrompt, model, temperature, max_tokens);
          break;
  ...
  app.post('/explain', async (req, res) => {
    try {
      ...
      const explanationPrompt = buildExplanationPrompt(code, language, question);
      let explanation;
      ...
      switch (provider) {
        ...
        case 'anthropic':
          ...
          explanation = await generateAnthropicResponse(explanationPrompt, model);
          break;
...
async function generateOpenAIResponse(prompt, model, temperature = 0.7, max_tokens = 2000) {
...
async function generateAnthropicResponse(prompt, model, temperature = 0.7, max_tokens = 2000) {
...
async function generateMockResponse(prompt, file_path) {
  ...
  const responses = [
    `I can help you with that! Based on your question about "${prompt.slice(0, 50)}...", here are some suggestions:
...
Let me know if you'd like specific examples for any of these!`,
    ...
    `I understand you're working on ${file_path ? `the file "${file_path}"` : 'your code'}. Here's my analysis:
...
async function generateCodeCompletion(prompt, model) {
  ...
  const completion = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: 'You are a code completion assistant. Provide only the code completion, no explanations.'
      },
...
This ${language || 'code'} snippet ${question ? `(regarding "${question}")` : ''} does the following:
