...
class AdvancedGitService:
    ...
    def get_commit_graph(self, max_count: int = 100, branch: Optional[str] = None) -> List[GitCommit]:
                    ...
                    if '|' in clean_line:
                        ...
                        if len(parts) >= 6:
                            commit = GitCommit(
                                ...
                                refs=parts[7].split(', ') if len(parts) > 7 and parts[7] else []
                            )
    ...
    def get_blame_info(self, file_path: str, start_line: int = 1, end_line: Optional[int] = None) -> List[BlameInfo]:
                ...
                if line.strip():
                    ...
                    if len(parts) >= 5:
                        ...
                        stash = GitStash(
                            index=index,
                            message=parts[1],
                            branch=parts[2].split(': ')[-1] if ': ' in parts[2] else parts[2],
    ...
    def apply_stash(self, stash_index: int = 0) -> bool:
        """Apply a stash"""
        try:
            result = self._run_git_command(["stash", "apply", f"stash@{{{stash_index}}}"])
            return result.returncode == 0
    ...
    def drop_stash(self, stash_index: int = 0) -> bool:
        """Drop a stash"""
        try:
            result = self._run_git_command(["stash", "drop", f"stash@{{{stash_index}}}"])
            return result.returncode == 0
    ...
    def resolve_conflict(self, file_path: str, resolution: str, content: Optional[str] = None) -> bool:
    ...
    def compare_branches(self, branch1: str, branch2: str) -> Dict[str, Any]:
        ...
        try:
            ...
            ahead_commits = [line.strip() for line in ahead_result.stdout.split('\n') if line.strip()]
            behind_commits = [line.strip() for line in behind_result.stdout.split('\n') if line.strip()]
