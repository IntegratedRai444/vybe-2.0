...
class DeploymentService:
    ...
    def _save_targets(self):
        ...
        data = {
            target_id: {k: v for k, v in asdict(target).items() if k != "id"}
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if deploy_result.get("success"):
                    ...
                    self._log_deployment(
                        deployment, f"Deployment failed: {deployment.error}"
                    )
    ...
    async def _deploy_to_provider(
        self, deployment: Deployment, target: DeploymentTarget, build_dir: Path
    ) -> Dict[str, Any]:
    ...
    async def _deploy_to_vercel(
        ...
        try:
            ...
            try:
                subprocess.run(
                    ["vercel", "--version"], check=True, capture_output=True, text=True
                )
            ...
            result = subprocess.run(
                cmd, cwd=str(build_dir), env=env, capture_output=True, text=True
            )
            ...
            return {"success": False, "error": f"Vercel deployment failed: {e.stderr}"}
    ...
    async def _deploy_to_netlify(
        ...
        try:
            ...
            return {"success": False, "error": f"Netlify deployment failed: {e.stderr}"}
    ...
    async def _deploy_to_aws(
        ...
        try:
            ...
            try:
                subprocess.run(
                    ["aws", "--version"], check=True, capture_output=True, text=True
                )
            except (subprocess.CalledProcessError, FileNotFoundError):
                self._log_deployment(deployment, "Please install AWS CLI first")
                return {"success": False, "error": "AWS CLI is not installed"}
            ...
            if "cloudfront_distribution_id" in target.config:
                ...
                cmd = [
                    ...
                    "create-invalidation",
                    "--distribution-id",
                    target.config["cloudfront_distribution_id"],
                    "--paths",
                    "/*",
                ]
            ...
            return {"success": False, "error": f"AWS deployment failed: {e.stderr}"}
    ...
    async def _deploy_custom(
        ...
        try:
            if "command" not in target.config:
                return {"success": False, "error": "No custom command specified"}
            ...
            return {"success": False, "error": f"Custom deployment failed: {str(e)}"}
