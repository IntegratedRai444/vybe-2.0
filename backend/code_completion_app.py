...
from ai.code_completion import CompletionItem, code_completion
...
async def get_completions(request: CompletionRequest):
    ...
    try:
        logger.info(
            f"Getting completions for {request.file_path} at position {request.cursor_pos}"
        )
...
async def health_check():
    ...
    return {"status": "ok", "service": "code-completion", "version": "1.0.0"}
