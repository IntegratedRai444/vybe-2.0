...
async def search_packages(
    query: str, language: str = "python", limit: int = 20, project_root: str = ""
):
    """
    ...
    try:
        ...
        results = await package_manager.search_packages(
            project_root=project_root, language=language, query=query, limit=limit
        )
        return results
...
async def install_package(install_request: PackageInstallRequest):
    ...
    try:
        ...
        return {
            "status": "success",
            "message": f"Package {install_request.package_name} installed successfully",
        }
...
async def uninstall_package(uninstall_request: PackageUninstallRequest):
    ...
    try:
        ...
        return {
            "status": "success",
            "message": f"Package {uninstall_request.package_name} uninstalled successfully",
        }
...
async def update_package(
    project_root: str, language: str, package_name: str, version: Optional[str] = None
):
    """
...
async def list_installed_packages(
    project_root: str, language: str, outdated: bool = False
):
    """
...
async def get_package_info(package_name: str, project_root: str, language: str):
    """
    Get detailed information about a package
    """
    try:
        package = await package_manager.get_package_info(
            project_root=project_root, language=language, package_name=package_name
        )
...
async def create_virtual_environment(project_root: str, language: str = "python"):
    """
...
async def audit_packages(project_root: str, language: str):
    """
    ...
    try:
        if language == "python":
            ...
            try:
                ...
                logger.warning(
                    "safety package not installed. Install with: pip install safety"
                )
                return {
                    "status": "warning",
