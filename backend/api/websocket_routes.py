...
from ..mcp.models import CodeIssue, FileChangeEvent, ScanResult
from ..mcp.service import MCPService, get_mcp_service
from ..services.websocket_manager import WebSocketEvent, websocket_manager
        ...
        while True:
            try:
                ...
                if message_type == "subscribe":
                    channel = (
                        ws_message.data.get("channel") if ws_message.data else None
                    )
                    if channel:
                ...
                # Handle ping/pong for keepalive
                    ...
                    await websocket_manager.send_personal_message(
                        client_id, WebSocketEvent.create_event("pong", {})
                    )
                ...
                logger.error(
                    f"Error processing message from {client_id}: {str(e)}",
                    exc_info=True,
                )
                try:
                    await websocket_manager.send_personal_message(
...
async def handle_custom_message(
    websocket: WebSocket, client_id: str, message: WebSocketMessage
):
    ...
    try:
        ...
        logger.error(
            f"Error handling message from {client_id}: {str(e)}", exc_info=True
        )
        await websocket_manager.send_personal_message(
            client_id,
            WebSocketEvent.create_event(
...
async def send_scan_result(result: ScanResult):
    ...
    await websocket_manager.broadcast(
        "scan_results",
        WebSocketEvent.create_event(
            "scan_result", result.dict() if hasattr(result, "dict") else result
        ),
    )
...
async def send_file_change(event: FileChangeEvent):
    ...
    await websocket_manager.broadcast(
        "file_changes",
        WebSocketEvent.create_event(
            "file_change", event.dict() if hasattr(event, "dict") else event
        ),
    )
        ...
        if hasattr(mcp, "_notify_scan_complete"):
            ...
            async def notify_with_ws(result: ScanResult):
                try:
                    ...
                    logger.error(
                        f"Error in WebSocket notification: {str(e)}", exc_info=True
                    )
