    ...
    try:
        ...
        if "environment" in target_dict:
            deployment_service.update_target(
                target.id, environment=target_dict["environment"]
            )
...
async def update_deployment_target(
    target_id: str, target_update: DeploymentTargetUpdate
):
    """
...
async def start_deployment(
    deployment_request: DeploymentStartRequest, background_tasks: BackgroundTasks
):
    """
...
async def list_deployments(limit: int = 10):
    ...
    try:
        ...
        all_deployments = sorted(
            active_deployments.values(), key=lambda d: d.created_at, reverse=True
        )
...
async def cancel_deployment(deployment_id: str):
    ...
    deployment.logs.append(
        f"[{datetime.utcnow().isoformat()}] Deployment was canceled by user"
    )
...
async def run_deployment(
    ...
    try:
        ...
        deployment.logs.append(
            f"[{datetime.utcnow().isoformat()}] Starting deployment to {target.name}..."
        )
        ...
        if deployment.status == DeploymentStatus.SUCCESS:
            deployment.logs.append(
                f"[{datetime.utcnow().isoformat()}] Deployment completed successfully"
            )
        else:
            deployment.logs.append(
                f"[{datetime.utcnow().isoformat()}] Deployment failed: {deployment.error}"
            )
        ...
        if deployment:
            ...
            deployment.updated_at = datetime.utcnow().isoformat()
            deployment.logs.append(
                f"[{datetime.utcnow().isoformat()}] Deployment error: {str(e)}"
            )
