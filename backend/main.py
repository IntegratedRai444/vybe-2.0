...
app = FastAPI(title="Vybe AI OS Backend")
...
def build_tree(root: Path, rel: Path = Path(""), max_depth: int = 10, current_depth: int = 0) -> FolderNode:
    ...
    for entry in entries:
        ...
        if (entry.name.startswith(".") or 
            entry.name in ["node_modules", "__pycache__", ".git", "dist", "build", ".next", ".vscode"]):
            continue
...
def chat(req: ChatRequest):
    ...
    try:
        ...
        gen_req = GenerateRequest(
            prompt=req.message,
            file_path=req.context or "chat.txt",
            model=None,
            top_k=3
        )
        ...
        return {
            "answer": f"Chat service unavailable: {str(e)}",
            "error": str(e)
        }
...
async def lsp_document_change(req: LSPDocumentRequest):
    ...
    try:
        await lsp_manager.did_change_document(req.file_path, req.content, req.version, req.language)
        return {"success": True}
    except Exception as e:
        raise HTTPException(400, str(e))
...
async def create_debug_session(req: DAPCreateSessionRequest):
    ...
    try:
        session_id = await dap_manager.create_session(req.language, req.program, req.args)
        if session_id:
...
async def get_threads(session_id: str):
    ...
    try:
        ...
        return {
            "threads": [
                {"id": t.id, "name": t.name} for t in threads
            ]
        }
    except Exception as e:
        raise HTTPException(400, str(e))
...
def get_commit_graph(req: GitRepoRequest, max_count: int = 100, branch: Optional[str] = None):
...
def get_blame_info(req: GitBlameRequest):
    ...
    try:
        git_service = get_advanced_git_service(req.repo_path)
        blame_info = git_service.get_blame_info(req.file_path, req.start_line, req.end_line)
...
def resolve_conflict(req: GitConflictResolveRequest):
    ...
    try:
        git_service = get_advanced_git_service(req.repo_path)
        success = git_service.resolve_conflict(req.file_path, req.resolution, req.content)
        return {"success": success}
    except Exception as e:
        raise HTTPException(400, str(e))
...
def get_replace_preview(req: ReplaceRequest):
    ...
    try:
        ...
        from search.advanced_search import SearchResult, SearchMatch
...
def execute_replace(req: ReplaceRequest):
    ...
    try:
        ...
        result = search_service.execute_replace(search_result, req.replacement, req.selected_matches)
        return result
    except Exception as e:
        raise HTTPException(400, str(e))
...
def get_hover_info(req: HoverRequest):
    ...
    try:
        result = code_intelligence.get_hover_info(req.code, req.cursor_pos, req.file_path)
        return result
    except Exception as e:
        return {
            "info": None,
...
def get_signature_help(req: SignatureRequest):
    ...
    try:
        result = code_intelligence.get_signature_help(req.code, req.cursor_pos, req.file_path)
        return result
    except Exception as e:
        return {
            "signatures": [],
...
def find_definition(req: DefinitionRequest):
    ...
    try:
        result = code_intelligence.find_definition(req.code, req.cursor_pos, req.file_path)
        return result
    except Exception as e:
        return {
            "locations": [],
...
def find_references(req: ReferencesRequest):
    ...
    try:
        result = code_intelligence.find_references(req.code, req.cursor_pos, req.file_path)
        return result
    except Exception as e:
        return {
            "references": [],
...
def set_breakpoint(req: BreakpointRequest):
    """Set a breakpoint"""
    try:
        result = debugger_service.set_breakpoint(req.session_id, req.line, req.condition)
        return result
    except Exception as e:
        return {"error": str(e), "success": False}
...
def update_setting(req: UpdateSettingRequest):
    ...
    try:
        if req.scope == "workspace" and req.workspace_path:
            success = settings_service.update_workspace_setting(req.workspace_path, req.key_path, req.value)
        else:
            success = settings_service.update_user_setting(req.key_path, req.value)
        
        return {"success": success, "message": "Setting updated" if success else "Failed to update setting"}
    except Exception as e:
        return {"error": str(e), "success": False}
...
def reset_settings(scope: str = "user"):
    ...
    try:
        ...
        return {"success": success, "message": "Settings reset" if success else "Failed to reset settings"}
    except Exception as e:
        return {"error": str(e), "success": False}
...
def import_settings(req: ImportSettingsRequest):
    ...
    try:
        ...
        return {"success": success, "message": "Settings imported" if success else "Failed to import settings"}
    except Exception as e:
        return {"error": str(e), "success": False}
...
def update_keybinding(req: UpdateKeybindingRequest):
    ...
    try:
        ...
        return {"success": success, "message": "Keybinding updated" if success else "Failed to update keybinding"}
    except Exception as e:
        return {"error": str(e), "success": False}
    ...
    try:
        ...
        for file_path in root_path.rglob('*'):
            if file_path.is_file():
                ...
                if (file_path.suffix in ['.pyc', '.exe', '.dll', '.so', '.dylib'] or
                    any(ignore in str(file_path) for ignore in ['.git', '__pycache__', 'node_modules'])):
                    continue
                
                # Apply include/exclude patterns
                if req.includePattern and not re.search(req.includePattern, str(file_path)):
                    continue
...
async def lsp_document_change(req: LSPDocumentRequest):
    """Notify LSP that document was changed"""
    try:
        await lsp_manager.did_change_document(req.file_path, req.content, req.version or 1, req.language)
        return {"success": True}
    except Exception as e:
...
async def lsp_completions(req: LSPPositionRequest):
    """Get LSP completions"""
    try:
        completions = await lsp_manager.get_completions(req.file_path, req.line, req.character, req.language)
        return {"completions": [
            {
                "label": c.label,
                "kind": c.kind,
...
async def lsp_hover(req: LSPPositionRequest):
    """Get LSP hover information"""
    try:
        hover = await lsp_manager.get_hover(req.file_path, req.line, req.character, req.language)
        return {"hover": hover}
    except Exception as e:
...
async def lsp_definition(req: LSPPositionRequest):
    """Get LSP definition"""
    try:
        definition = await lsp_manager.goto_definition(req.file_path, req.line, req.character, req.language)
        return {"definition": definition}
    except Exception as e:
...
async def lsp_references(req: LSPPositionRequest):
    """Get LSP references"""
    try:
        references = await lsp_manager.find_references(req.file_path, req.line, req.character, req.language)
        return {"references": references}
    except Exception as e:
