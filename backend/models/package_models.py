...
class PackageInstallRequest(PackageBase):
    ...
    dev: bool = Field(False, description="Whether to install as a development dependency")
...
class PackageInfo(BaseModel):
    ...
    requires: List[str] = Field(default_factory=list, description="List of required packages")
    required_by: List[str] = Field(default_factory=list, description="Packages that depend on this one")
...
class PackageVulnerability(BaseModel):
    ...
    fixed_versions: List[str] = Field(default_factory=list, description="Versions with fixes")
...
class PackageAuditResult(BaseModel):
    ...
    vulnerabilities: List[PackageVulnerability] = Field(default_factory=list, description="List of vulnerabilities")
...
class PackageManagerInfo(BaseModel):
    ...
    package_file: str = Field(..., description="Path to package file (e.g., requirements.txt, package.json)")
    lock_file: Optional[str] = Field(None, description="Path to lock file if exists")
    virtual_env: Optional[str] = Field(None, description="Path to virtual environment if used")
    is_initialized: bool = Field(False, description="Whether the package manager is initialized")
