...
class GitFile(BaseModel):
    ...
    original_path: Optional[str] = Field(
        None,
        description="Original path (for renamed/copied files)"
    )
...
class GitBranch(BaseModel):
    ...
    upstream: Optional[str] = Field(
        None,
        description="Upstream branch name (if tracked)"
    )
    ...
    last_commit: Optional[Dict[str, Any]] = Field(
        None,
        description="Last commit on this branch"
    )
...
class GitRepositoryInfo(BaseModel):
    ...
    last_commit: Optional[Dict[str, Any]] = Field(
        None,
        description="Last commit information"
    )
...
class GitCommitCreate(BaseModel):
    ...
    files: Optional[List[str]] = Field(
        None,
        description="Specific files to include in the commit"
    )
    all_files: bool = Field(
        False,
        description="Whether to include all changed files"
    )
    amend: bool = Field(
        False,
...
class GitBranchCreate(BaseModel):
    ...
    start_point: Optional[str] = Field(
        None,
        description="Starting point for the new branch (commit, branch, or tag)"
    )
...
class GitConflictResolution(BaseModel):
    ...
    resolution_type: str = Field(
        "theirs",
        description="Resolution type (ours, theirs, custom)"
    )
...
class GitStash(BaseModel):
    ...
    files: List[Dict[str, Any]] = Field(
        default_factory=list,
        description="List of stashed files"
    )
...
class GitTag(BaseModel):
    ...
    tagger: Optional[Dict[str, str]] = Field(
        None,
        description="Tagger information (name, email, date)"
    )
...
class GitDiff(BaseModel):
    ...
    similarity_index: Optional[int] = Field(
        None,
        description="Similarity index for renames/copies"
    )
