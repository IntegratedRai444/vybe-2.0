...
class DeploymentStartRequest(BaseModel):
    ...
    commit_hash: Optional[str] = Field(
        None,
        description="Git commit hash to deploy (defaults to HEAD)"
    )
    branch: Optional[str] = Field(
        None,
        description="Git branch to deploy (defaults to current branch)"
    )
    environment: Optional[Dict[str, str]] = Field(
        None,
        description="Additional environment variables for this deployment"
    )
...
class DeploymentStatusResponse(BaseModel):
    ...
    completed_at: Optional[str] = Field(
        None,
        description="Deployment completion timestamp (if finished)"
    )
    url: Optional[HttpUrl] = Field(
        None,
        description="URL of the deployed application (if available)"
    )
...
class DeploymentProviderInfo(BaseModel):
    ...
    icon: Optional[str] = Field(
        None,
        description="URL to provider's icon"
    )
    ...
    supports_preview: bool = Field(
        False,
        description="Whether the provider supports preview deployments"
    )
...
class DeploymentEnvironment(BaseModel):
    ...
    auto_deploy: bool = Field(
        False,
        description="Whether to automatically deploy when the branch is updated"
    )
...
class DeploymentSettings(BaseModel):
    ...
    hooks: List[DeploymentHook] = Field(
        default_factory=list,
        description="Deployment webhooks"
    )
