...
class FileHandler:
    ...
    def __init__(self):
        self.supported_text_extensions = {
            ...
            '.php', '.java', '.cpp', '.c', '.h', '.hpp', '.cs', '.rb', '.go',
            '.rs', '.swift', '.kt', '.scala', '.sh', '.bat', '.ps1', '.dockerfile',
            '.gitignore', '.env', '.ini', '.cfg', '.conf', '.toml', '.lock'
        }
    ...
    def read_file(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not self.is_text_file(str(path)):
                return {"error": "File is not a text file", "content": None, "binary": True}
        ...
        try:
            ...
            if path.exists():
                if path.is_dir():
                    ...
                    return {"success": False, "error": "Path exists but is not a folder"}
                ...
                for item in sorted(path.iterdir()):
                    ...
                    try:
                        ...
                        files.append({
                            ...
                            "is_text": self.is_text_file(str(item)) if item.is_file() else False
                        })
                        ...
                        try:
                            for child in sorted(path.iterdir()):
                                ...
                                if (child.name.startswith('.') or 
                                    child.name in ['node_modules', '__pycache__', '.git', 'dist', 'build']):
                                    continue
    ...
    def search_files(self, root_path: str, query: str, max_results: int = 100) -> Dict[str, Any]:
