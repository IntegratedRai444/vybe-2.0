...
class AdvancedSearchService:
    ...
    def _search_in_file(
        self, file_path: Path, pattern: Pattern, context_lines: int
    ) -> List[SearchMatch]:
    ...
    def get_replace_preview(
        self, search_result: SearchResult, replacement: str, max_previews: int = 100
    ) -> List[ReplacePreview]:
    ...
    def execute_replace(
        ...
        if selected_matches is None:
            ...
            matches_to_replace = [
                search_result.matches[i]
                for i in selected_matches
                if i < len(search_result.matches)
            ]
        ...
        results = {"files_modified": 0, "replacements_made": 0, "errors": []}
                        ...
                        if search_result.pattern_type == "regex":
                            ...
                            if search_result.case_sensitive:
                                new_line = line.replace(
                                    match.match_text, replacement, 1
                                )
                            else:
    ...
    def export_search_results(
        self, search_result: SearchResult, format: str = "json"
    ) -> str:
        ...
        try:
            if format == "json":
                ...
                for match in search_result.matches:
                    lines.append(
                        f"{match.file_path}:{match.line_number}:{match.column_start}"
                    )
