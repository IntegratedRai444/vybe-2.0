...
class CodeIntelligence:
    ...
    def __init__(self):
        ...
        self._symbol_cache: Dict[
            Tuple[str, str], Tuple[float, List[CompletionItem]]
        ] = {}
        self._ai_completion_cache: Dict[
            Tuple[str, str, str], Tuple[float, List[CompletionItem]]
        ] = {}
    ...
    async def get_completions(
        ...
        try:
            ...
            if current_word:
                completions = [
                    c for c in completions if current_word.lower() in c["label"].lower()
                ]
            ...
            if context:
                if "imports" in context and language == "python":
                    for imp in context["imports"]:
                        if isinstance(imp, str) and (
                            not current_word or current_word.lower() in imp.lower()
                        ):
                            completions.append(
                                {
                                    "label": imp,
            ...
            if language == "python":
                completions = self._get_python_completions(
                    code, current_line, current_word, cursor_pos
                )
            elif language in ["javascript", "typescript"]:
                completions = self._get_js_completions(
                    code, current_line, current_word, cursor_pos
                )
            else:
    ...
    def get_hover_info(
        self, code: str, cursor_pos: int, file_path: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"info": info, "word": word, "language": language, "success": True}
    ...
    def find_definition(
        ...
        try:
            ...
            return {"locations": locations, "symbol": word, "success": True}
    ...
    def find_references(
        ...
        try:
            ...
            return {"references": references, "symbol": word, "success": True}
    ...
    def _get_python_completions(
        self, code: str, current_line: str, current_word: str, cursor_pos: int
    ) -> List[Dict[str, Any]]:
        ...
        # Add common imports
        if current_line.strip().startswith("import") or current_line.strip().startswith(
            "from"
        ):
            common_modules = [
                "os",
    ...
    def _get_generic_completions(
        self, current_word: str, language: str
    ) -> List[Dict[str, Any]]:
    ...
    def _get_symbol_info(
        self, symbol: str, language: str, code: str
    ...
    def _get_function_signatures(
        self, func_name: str, language: str
    ) -> List[Dict[str, Any]]:
                ...
                signatures.append(
                    {
                        ...
                        "parameters": [
                            ...
                            {
                                "label": "sep",
                                "documentation": "String inserted between values",
                            },
                            {
                                "label": "end",
                                "documentation": "String appended after the last value",
                            },
                            {
                                "label": "file",
                                "documentation": "File object to write to",
                            },
                            {
                                "label": "flush",
    ...
    def _find_symbol_definition(
        self, symbol: str, code: str, file_path: str, language: str
    ) -> List[Dict[str, Any]]:
