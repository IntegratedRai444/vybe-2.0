...
class FileChangeHandler(FileSystemEventHandler):
    ...
    def _create_event(
        self, event_type: str, src_path: str, is_directory: bool = False
    ) -> FileChangeEvent:
    ...
    def on_moved(self, event: FileSystemEvent):
        if self._should_handle(event.src_path) or (
            event.dest_path and self._should_handle(event.dest_path)
        ):
            change_event = self._create_event(
                "moved", event.src_path, event.is_directory
            )
...
class FileWatcher:
    ...
    def __init__(
        self, scanner: CodeScanner, config: Optional[RealTimeScanConfig] = None
    ):
    ...
    def _should_process_event(self, event: FileChangeEvent) -> bool:
        ...
        if event.src_path.endswith("~") or (
            event.src_path.startswith(".") and "~" in event.src_path
        ):
            return False

        # Skip files that don't match include patterns
        if not any(
            fnmatch.fnmatch(event.src_path, p) for p in self.config.include_patterns
        ):
            return False

        # Skip files that match exclude patterns
        if any(
            fnmatch.fnmatch(event.src_path, p) for p in self.config.exclude_patterns
        ):
            return False
    ...
    async def _process_scan_queue(self):
        ...
        while self._is_running:
            try:
                ...
                if pending_scan and (current_time - last_scan_time) >= (
                    ...
                    try:
                        ...
                        logger.error(
                            f"Error processing file change: {e}", exc_info=True
                        )
...
async def get_file_watcher(
    scanner: Optional[CodeScanner] = None, config: Optional[RealTimeScanConfig] = None
) -> FileWatcher:
