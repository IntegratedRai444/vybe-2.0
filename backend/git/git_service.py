...
class GitService:
    ...
    def get_status(self) -> GitRepository:
        ...
        repo.untracked_files = [f for f in repo.files if f.status == FileStatus.UNTRACKED]
    ...
    def _get_conflicts(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for file_path in unmerged.split('\n'):
                ...
                file_content = (self.repo_path / file_path).read_text(encoding='utf-8', errors='ignore')
    ...
    def _list_branches(self) -> List[GitBranch]:
        ...
        # Get local branches
        local_branches = self._run_git(['branch', '--list', '--format=%(refname:short)']).split('\n')
        for branch in local_branches:
            ...
            try:
                upstream = self._run_git(['rev-parse', '--abbrev-ref', f'{branch}@{{upstream}}'])
                
                # Get ahead/behind counts
                counts = self._run_git(['rev-list', '--left-right', '--count', f'{branch}...{branch}@{{upstream}}'])
        ...
        remote_branches = self._run_git(['branch', '-r', '--list', '--format=%(refname:short)']).split('\n')
    ...
    def create_tag(self, name: str, message: str = '', commit_hash: str = 'HEAD') -> bool:
        """Create a new tag"""
        try:
            if message:
        ...
        try:
            ...
            for line in stash_list.split('\n'):
                ...
                if match:
                    ...
                    stashes.append({
                        'index': int(index),
                        'branch': branch.split(' ')[-1],  # Extract branch name from "On branch name"
                        'message': message or ''
                    })
