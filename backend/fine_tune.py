...
class ModelFineTuner:
    ...
    def start_fine_tuning(
        self, training_data: List[Dict[str, str]], model_name: str, epochs: int = 3
    ) -> bool:
...
def main():
    ...
    parser.add_argument(
        "--data", type=str, required=True, help="Path to training data JSONL file"
    )
    parser.add_argument(
        "--model", type=str, default="llama2", help="Base model to fine-tune"
    )
    parser.add_argument(
        "--name", type=str, required=True, help="Name for the fine-tuned model"
    )
    parser.add_argument(
