...
class AnthropicProvider(BaseModelProvider):
    ...
    async def embed(self, text: str, model: str = "claude-3-opus-20240229") -> List[float]:
        """
        ...
        try:
            ...
            response = await self.client.messages.create(
                model=model,
                messages=[{"role": "user", "content": f"Return a numerical embedding for the following text: {text}"}],
                max_tokens=1024,
                temperature=0.0
            )
