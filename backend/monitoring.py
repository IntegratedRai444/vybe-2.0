...
AI_REQUESTS = Counter(
    "ai_requests_total", "Total AI API requests", ["provider", "model", "status"]
)

AI_REQUEST_LATENCY = Histogram(
    "ai_request_duration_seconds", "AI request latency", ["provider", "model"]
)
    ...
    def decorator(func: Callable[..., Awaitable[Any]]):
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                AI_REQUEST_LATENCY.labels(provider=provider, model=model).observe(
                    duration
                )
